...
public class SynWaveDataFDView extends SurfaceView implements SurfaceHolder.Callback 
{
	 ...
	 */
	public SynWaveDataFDView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init( context );
	}
	 ...
	 * @param context Context of the application
	 */
	public SynWaveDataFDView(Context context) {
		super(context);
		init( context );
	}
	...
	public void drawFD() {
		if(isSurfaceCreated) {	
			...
			if( canvas != null ) {
				...
				drawAxisX(SynWaveDataFD.MAX_F);
				drawAxisY();
				paint.setColor(Color.RED);
				drawSignalGraph(SynWaveDataFD.transformedSignal);
			}
	...
	private void drawAxisX(double samplingRate ) {
		...
		if(samplingRate >= 1000) {
			...
			float xInterval = (float)(scrWidth-100) / maxFrequencyInt;
			...
			if(maxFrequencyInt != (int)maxFrequencyInt) {
				canvas.drawText(String.valueOf(maxFrequencyInt), scrWidth-50, scrHeight-25, textPaint);
			}
		}else {
			int xInterval = (scrWidth-100) / 10;
			int sampleInterval = (int) (samplingRate / 10);
			paint.setStrokeWidth(1);
			for(int i=0; i<=10; i++) {
				canvas.drawText(String.valueOf(sampleInterval*i), 50+xInterval*i, scrHeight-25, textPaint);
				canvas.drawLine(50+xInterval*i, scrHeight-50, 50+xInterval*i, 50, paint);
			}
			textPaint.setTextSize(20);
			canvas.drawText("Hz", scrWidth-50, scrHeight-50, textPaint);
			textPaint.setTextSize(30);
		}
	}
