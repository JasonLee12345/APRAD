...
public class SynWaveDataSView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static final int    SIGNALS_HEIGHT 		= SynWaveDataS.sigInterval / 2;
	 ...
	 */
	public SynWaveDataSView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init( context );
	}
	 ...
	 * @param context Context of the application
	 */
	public SynWaveDataSView(Context context) {
		super(context);
		init( context );
	}
	...
	public void drawS() {
		if(isSurfaceCreated) {	
			...
			if( canvas != null ) {
				...
				drawAxisX();
				drawAxisY(SynWaveDataS.samplingRate);
				drawSignalGraph(SynWaveDataS.transformedSignal);
			}
	...
	private void drawAxisY(double samplingRate) {
		...
		if(samplingRate >= 1000) {
			float maxFrequencyInt = (float) samplingRate / 1000 /2;
			float yInterval = (scrHeight-100) / maxFrequencyInt;
			paint.setStrokeWidth(1);
			for(int i=0; i<=maxFrequencyInt; i++) {
				canvas.drawText(String.valueOf(i), 10, scrHeight-50-i*yInterval, textPaint);
			}
			...
			//canvas.drawText("Time Span: "+String.valueOf(SynWaveDataS.timeSpan)+" s", scrWidth-300, 25, textPaint);
			if(maxFrequencyInt != (int)maxFrequencyInt) {
				canvas.drawText(String.valueOf(maxFrequencyInt), 10, 50, textPaint);
			}
		}else {
			int yInterval = (scrHeight-100) / 10;
			int sampleInterval = (int) (samplingRate / 10);
			paint.setStrokeWidth(1);
			for(int i=0; i<=10; i++) {
				canvas.drawText(String.valueOf(sampleInterval*i), 10, scrHeight-50-yInterval*i, textPaint);
			}
			textPaint.setTextSize(20);
			canvas.drawText("Hz", 20, 25, textPaint);
			textPaint.setTextSize(30);
		}
		/*
	...
	private void drawSignalGraph(double[][] transformedSigs) {
		...
		float widthScalar = ((float)scrWidth-100)*scalePoint / SIGNALS_WIDTH;
		double signalValue;
		int colorAmplitude;
